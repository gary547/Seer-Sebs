// v2 SERP visibility / CTR deflator helper — browser mirror.
//
// This file MUST stay algorithmically identical to
// `supabase/functions/_shared/serp-visibility-v2.ts` (Deno). Both are pure
// functions; keeping a browser copy avoids importing Deno-URL-style modules
// from the frontend bundle. Update both files together and re-run the vitest
// suite (`src/test/serpVisibility.test.ts`) plus the Deno tests.
