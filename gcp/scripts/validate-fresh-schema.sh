    ...
    psql -U seer_owner -d seer -Atqc "
      SELECT
        ...
        EXISTS (
          SELECT 1
          FROM pg_constraint
          WHERE conrelid = 'ctr_curves'::regclass
            AND conname = 'ctr_curves_device_check'
        ...
        to_regclass('public.keyword_cluster_members_keyword_lookup_idx') IS NOT NULL
          ...
          AND to_regclass('public.pipeline_stage_output_chunks') IS NOT NULL
          AND has_table_privilege('seer_worker_local', 'pipeline_stage_output_chunks', 'SELECT')
          AND has_table_privilege('seer_worker_local', 'pipeline_stage_output_chunks', 'INSERT')
          AND has_table_privilege('seer_worker_local', 'pipeline_stage_output_chunks', 'DELETE')
          AND NOT has_table_privilege('seer_api_local', 'pipeline_stage_output_chunks', 'SELECT')
...
expected_migrations+=",036_pipeline_stage_output_chunks"
applied_migrations="$(
  docker exec "$container_name" \
    psql -U seer_owner -d seer -Atqc "
